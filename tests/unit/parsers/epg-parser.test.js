...
<tv>
  ...
  <programme start="20240101120000 +0000" stop="20240101130000 +0000" channel="ch1">
    ...
    it('accepts a comment after the root element', () => {
      // XML kok elemandan sonra yorum/isleme yonergesine izin verir; bazi
      // saglayicilar `</tv>` ardina imza yorumu ekliyor.
      const xml = '<tv><channel id="ch1"><display-name>News</display-name></channel></tv>\n<!-- yayinakisi -->\n';
      expect(() => parser.parse(xml)).not.toThrow();
      expect(parser.parse(xml).channels).toHaveLength(1);
    });

    it('accepts a processing instruction after the root element', () => {
      const xml = '<tv><channel id="ch1"><display-name>News</display-name></channel></tv><?generated by provider?>';
      expect(() => parser.parse(xml)).not.toThrow();
    });

    it('still rejects truncation hidden behind a trailing comment', () => {
      const xml = '<tv><channel id="ch1"><display-name>News</display-name></channel><!-- kesildi -->';
      expect(() => parser.parse(xml)).toThrow('</tv> kapanış etiketi bulunamadı');
    });
...
<tv>
  ...
  <programme start="20240101120000 +0000" stop="20240101130000 +0000" channel="ch1">
    ...
    it('accepts a stream whose root close is followed by a comment', async () => {
      const xml = '<tv><channel id="ch1"><display-name>News</display-name></channel></tv>\n<!-- yayinakisi -->\n';
      const items = [];
      for await (const item of parser.parseStream(createStream(xml))) items.push(item);
    ...
    it('accepts a trailing comment split across chunks', async () => {
      const items = [];
      for await (const item of parser.parseStream(createChunkedStream([
        '<tv><channel id="ch1"><display-name>News</display-name></channel></tv>\n<!-- yayin',
        'akisi imzasi -->\n',
      ]))) items.push(item);
      expect(items).toHaveLength(1);
    });

    it('still rejects a truncated stream that ends with a comment', async () => {
      const xml = '<tv><channel id="ch1"><display-name>News</display-name></channel><!-- kesildi -->';
      await expect(async () => {
